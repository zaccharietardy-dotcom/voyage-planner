...
function fixOpeningHoursViolations(
  days: TripDay[],
  startDate: string,
  repairs: RepairAction[],
  unresolvedViolations: string[]
        ...
        for (const otherItem of otherDay.items) {
          ...
          if (violatingFitsOtherDay && otherFitsThisDay) {
            ...
            if (dayIdx >= 0 && otherIdx >= 0) {
              ...
              // Swap items
              day.items[dayIdx] = { ...otherItem, startTime: violatingTime.start, endTime: violatingTime.end, dayNumber: violatingTime.dayNum, orderIndex: violatingTime.orderIdx };
              otherDay.items[otherIdx] = { ...violating, startTime: otherTime.start, endTime: otherTime.end, dayNumber: otherTime.dayNum, orderIndex: otherTime.orderIdx };
...
function ensureMustSees(
  ...
  for (const mustSee of mustSees) {
    ...
    for (const day of days) {
      ...
      // Find lowest-scored non-must-see activity to evict
      ...
      if (evictCandidates.length > 0) {
        ...
        if (idx >= 0) {
          ...
          day.items[idx] = {
            ...
            duration: mustSee.duration || 60,
            rating: mustSee.rating,
            mustSee: true,
            description: mustSee.description || day.items[idx].description,
