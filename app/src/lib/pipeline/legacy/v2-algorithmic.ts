 ...
 * Moved from index.ts to keep the main orchestrator lean.
 */
...
import { balanceDays } from '../step6-balance';
...
// Pipeline Event emit helper (local copy — index.ts owns the canonical version)
...
export async function generateTripV2Algorithmic(
  ...
  const clusters = clusterActivities(
    ...
    data.destCoords,
    densityProfile,
    preferences.startDate.toISOString().split('T')[0] // Pass startDate as YYYY-MM-DD string
  );
